...
lsmkv::Status VersionRecord::Decode(const std::string& bytes, VersionRecord* out) {
    ...
    if (!lsmkv::GetFixed64(&input, &rec.start_ts) ||
        ...
        return lsmkv::Status::Corruption("version: truncated header");
    }
    if (input.empty()) {
        return lsmkv::Status::Corruption("version: missing flags");
    }
    rec.is_tombstone = (input.data()[0] != 0);
    input.remove_prefix(1);
    lsmkv::Slice payload;
    if (!lsmkv::GetLengthPrefixedSlice(&input, &payload)) {
        return lsmkv::Status::Corruption("version: bad payload");
    }
    if (!input.empty()) {
        return lsmkv::Status::Corruption("version: trailing bytes");
    }
...
lsmkv::Status DecodePkFromKey(const std::string& hex, Value* out) {
    std::string raw;
    if (!FromHex(hex, &raw)) {
        return lsmkv::Status::Corruption("bad pk hex");
    }
...
MvccStore::MvccStore(lsmkv::DB* db) : db_(db) {}
...
lsmkv::Status MvccStore::GetVersion(const std::string& table, const Value& pk,
    ...
    std::string bytes;
    auto st = db_->Get(lsmkv::ReadOptions(), VersionKey(table, pk_key, start_ts), &bytes);
    if (!st.ok()) return st;
...
lsmkv::Status MvccStore::GetLatestStartTs(const std::string& table, const Value& pk,
    ...
    const std::string pk_key = EncodePkForKey(pk);
    std::string bytes;
    auto st = db_->Get(lsmkv::ReadOptions(), RowHeadKey(table, pk_key), &bytes);
    if (!st.ok()) return st;
    if (bytes.size() != 8) {
        return lsmkv::Status::Corruption("row head: bad length");
    }
...
lsmkv::Status MvccStore::GetRow(const std::string& table, const Value& pk,
    ...
    // Walk prev_ts chain from newest toward oldest.
    while (ts != 0) {
        VersionRecord rec;
        st = GetVersion(table, pk, ts, &rec);
        if (!st.ok()) return st;

        if (IsVisible(rec, snapshot)) {
            if (rec.is_tombstone) {
                return lsmkv::Status::NotFound("row deleted at snapshot");
            }
            return Row::Decode(rec.payload, out);
        }
        ts = rec.prev_ts;
    }
    return lsmkv::Status::NotFound("no visible version");
}

lsmkv::Status MvccStore::PutVersion(const std::string& table, const Value& pk,
                                    const VersionRecord& rec) {
    const std::string pk_key = EncodePkForKey(pk);
    auto st = db_->Put(lsmkv::WriteOptions(), VersionKey(table, pk_key, rec.start_ts),
                       rec.Encode());
    if (!st.ok()) return st;
...
lsmkv::Status MvccStore::CloseVersion(const std::string& table, const Value& pk,
    ...
    VersionRecord rec;
    auto st = GetVersion(table, pk, start_ts, &rec);
    if (!st.ok()) return st;
    rec.end_ts = end_ts;
    const std::string pk_key = EncodePkForKey(pk);
