...
public:
    ...
    // After bind, ToString() reflects the bare names.
    lsmkv::Status Bind(const BindContext& ctx);

    // Walk column leaves; *name is rewritten in place and column_index_ reset.
    // Used to normalize HAVING refs (qual.col / SUM(u.x) → bare / SUM(x)).
    lsmkv::Status MapColumnNames(
